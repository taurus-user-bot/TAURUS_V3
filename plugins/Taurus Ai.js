...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('taurus') && conf.TALKING_TAURUS !== 'true') {        
...
if (conf.LANG == 'ML') {
    ...
    already_on = 'Taurus കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'Taurus AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'Taurus പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി Taurus സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'taurus ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.taurus on / off' }, (async (message, match) => {
    var taurus_status = `${conf.TALKING_TAURUS}`
    if (match[1] == 'on') {
        if (taurus_status == 'true') {
